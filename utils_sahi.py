...
def get_video_sahi_prediction(video_path, detection_model, slice_height=256, slice_width=256, overlap_height_ratio=0.2, overlap_width_ratio=0.2, output_video_path=None):
    ...
    if output_video_path:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')  # You can choose a different codec like 'mp4v'
    ...
    while True:
        ...
        # Merge overlapping bounding boxes from the prediction
        object_prediction_list = result.object_prediction_list
        object_prediction_list = merge_overlapping_boxes(object_prediction_list)
        
        # Annotate the frame with the number of detected objects
        ...
        # Press 'q' to quit the video processing early (optional)
...
def get_video_prediction_with_fallback(video_path, detection_model,yolo_model,  slice_height=256, slice_width=256, overlap_height_ratio=0.2, overlap_width_ratio=0.2, output_video_path=None,dis_thres=100):
    ...
    # If an output path is provided, initialize a video writer to save the output
    if output_video_path:
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # You can choose a different codec like 'mp4v'
        out = cv2.VideoWriter(output_video_path, fourcc, fps, (frame_width, frame_height))
    yolocnt = 0
    sahicnt = 0
    zerocnt = 0
    ...
    while True:
        ...
        if result_yolo.boxes.shape[0] == 0:  # If YOLO detects nothing
            ...
            # tmplist = []
            # for object_prediction in object_prediction_list:
            #     if dis(last_pos,get_center(object_prediction.bbox)) < dis_thres:
            #         tmplist.append(object_prediction)
            # object_prediction_list = tmplist
            # Annotate the frame with the number of detected objects
